...
def stateStr(state):
    ...
    """
    state_names = {SAFE: "Safe", LOST: "Loss", LEAKED: "Leak", STOLEN: "Theft"}
    state_strings = [state_names[key_state] for key_state in state]
    return "[" + ", ".join(state_strings) + "]"


def walletDifferingScenarios(
    wallet1,
    wallet2,
    keyCount,
    keyStateProbabilities1=None,
    keyStateProbabilities2=None,
):
    ...
    print(
        f"Difference (wallet1 - wallet2): {wallet1_total_prob - wallet2_total_prob:.6f}"
    )
    print("=" * 80)
